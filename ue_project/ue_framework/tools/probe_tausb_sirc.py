...
from ue_framework.methods.bsc_rc_gr_probe import canonical_hash
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the TAUSB-SIRC matched surrogate mechanism probe."
    )
    ...
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate the frozen schema without loading model, data, or artifacts.",
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Run the explicitly non-evidentiary local mechanical smoke path.",
    )
...
def load_config(path: Path) -> dict:
    ...
    if not isinstance(config, dict):
        raise ValueError("SIRC config must be a YAML mapping.")
