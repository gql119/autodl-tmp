...
def generate_delta(cfg: Dict, manifest: Dict, names: Sequence[str], target_id: int, output_root: Path):
    ...
    for epoch in range(int(cfg["poison"]["generation_epochs"])):
        ...
        for _image_ids, clean_images, batch in loader:
            ...
            if application.target_instances > 0 and losses["total_loss"].requires_grad:
                losses["total_loss"].backward()
                gradient_norm = (
                    float(perturbation.delta_object.grad.norm())
                    if perturbation.delta_object.grad is not None
                    else 0.0
                )
        ...
        row.update({key: float(np.mean(values)) for key, values in epoch_values.items()})
        row.update(perturbation.statistics())
        row.update(
            {
                "non_target_cls_drift": None,
                "non_target_box_drift": None,
                "non_target_assignment_drift": None,
                "gradient_norm_preserve": 0.0,
                "target_preserve_gradient_cosine": None,
                "classwise_drift": {},
...
def train_and_evaluate(cfg: Dict, manifest: Dict, names: Sequence[str], target_id: int, poison: Dict, output_root: Path):
    ...
    full = _val(victim, data_yaml, clean_root / "val.txt", eval_cfg, f"stage1_seed{seed}_clean_val")
    free = _val(victim, data_yaml, clean_root / "val_person_free.txt", eval_cfg, f"stage1_seed{seed}_person_free")
    cooccur = _val(victim, data_yaml, clean_root / "val_person_cooccur.txt", eval_cfg, f"stage1_seed{seed}_cooccur")
    poison_train = _val(victim, data_yaml, Path(poison["train_list"]), eval_cfg, f"stage1_seed{seed}_poison_train")
...
def main() -> None:
    ...
    if args.seed is not None:
        ...
        cfg["paths"]["output_root"] = str(configured_root.parent / f"stage1_seed{args.seed}")
    ...
    seed = int(cfg["experiment"]["seed"])
    write_json(results_root / f"stage1_seed{seed}_metrics.json", result)
    with (results_root / f"stage1_seed{seed}_per_class.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["class_id", "class_name", "is_target", "clean_ap50", "stage1_ap50", "drop"])
