...
CHECKOUT="/root/autodl-tmp/tausb-dgcaip/checkouts/${EXECUTION_COMMIT}-d0"
SESSION="tausb-dgcaip-d0-s0-r1"
PYTHON_BIN="/root/miniconda3/bin/python"
ARTIFACT_ROOT="/root/autodl-tmp/tausb-dgcaip-runs/TAUSB-SDH-DGCAIP-S0-R1-D0"
CONTROL_ROOT="/root/autodl-tmp/tausb-dgcaip-control/TAUSB-SDH-DGCAIP-S0-R1-D0"
CACHE_ROOT="/root/autodl-tmp/tausb-dgcaip-cache/TAUSB-SDH-DGCAIP-S0-R1-D0"
TMP_ROOT="/root/autodl-tmp/tausb-dgcaip-tmp/TAUSB-SDH-DGCAIP-S0-R1-D0"
OUTER_LOG="/root/autodl-tmp/tausb-dgcaip-logs/TAUSB-SDH-DGCAIP-S0-R1-D0.log"
PRELAUNCH_FAILURE_LOG="/root/autodl-tmp/tausb-dgcaip-logs/TAUSB-SDH-DGCAIP-S0-R1-D0-prelaunch-failure.log"
FAILED_LINE="unknown"
...
shutdown_on_prelaunch_failure() {
  ...
  if mountpoint -q "${REQUIRED_STORAGE_ROOT}"; then
    ...
    {
      printf 'schema=tausb.dgcaip-d0-prelaunch-failure.v1\n'
      printf 'exit_code=%q\n' "${rc}"
  ...
  echo "[DGCAIP-D0-Launch] prelaunch failed with exit ${rc}; requesting shutdown" >&2
  sync || true
...
WRAPPER="${CHECKOUT}/research_workspace/experiments/TAUSB-SDH-DGCAIP-S0-E20/pre_run/dgcaip_d0_controller_v1.sh"
CONFIG_PATH="${CHECKOUT}/ue_project/ue_framework/configs/tausb_sdh_dgcaip_d0_v2.yaml"
test -f "${WRAPPER}"
test -f "${CONFIG_PATH}"

printf -v TMUX_COMMAND \
  'env PYTHON_BIN=%q REPOSITORY_ROOT=%q REQUIRED_STORAGE_ROOT=%q EXPECTED_COMMIT=%q CONFIG_PATH=%q ARTIFACT_ROOT=%q CONTROL_ROOT=%q CACHE_ROOT=%q TMP_ROOT=%q /bin/bash %q > %q 2>&1' \
  "${PYTHON_BIN}" "${CHECKOUT}" "${REQUIRED_STORAGE_ROOT}" \
  "${EXECUTION_COMMIT}" "${CONFIG_PATH}" "${ARTIFACT_ROOT}" \
  "${CONTROL_ROOT}" "${CACHE_ROOT}" "${TMP_ROOT}" "${WRAPPER}" \
  "${OUTER_LOG}"

tmux new-session -d -s "${SESSION}" "${TMUX_COMMAND}"
trap - ERR EXIT INT TERM
echo "[DGCAIP-D0-Launch] handed off to tmux session ${SESSION}"
echo "[DGCAIP-D0-Launch] outer log: ${OUTER_LOG}"
echo "[DGCAIP-D0-Launch] status: ${CONTROL_ROOT}/controller_status.json"
