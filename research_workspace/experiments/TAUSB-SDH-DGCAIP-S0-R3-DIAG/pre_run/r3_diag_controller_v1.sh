...
: "${CONFIG_PATH:?reviewed R3 config is required}"
: "${ARTIFACT_ROOT:?unique R3 artifact root is required}"
...
assert config["spec"]["spec_id"] == "TAUSB-SDH-DGCAIP-R3-DIAG-v1"
...
if [[ "${remaining}" -le 0 ]]; then
  echo "[DGCAIP-R3] hard cap exhausted during initialization" >&2
  exit 124
fi
