...
finish() {
  ...
  if [[ ${rc} -eq 0 ]]; then
    write_guard_state "completed" "sb25_hiding_finished_shutdown_requested" || true
  else
    write_guard_state "failed" "sb25_hiding_exit_${rc}_shutdown_requested" || true
  fi
...
mark_stage "hiding_pilot" "sb25_single_secret_no_mechanism_no_victim_no_eot_no_jnd"
...
snapshot_evidence
mark_stage "completed" "sb25_hiding_and_evidence_snapshot_complete"
