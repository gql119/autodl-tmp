...
exp_root="${data_root}/tausb-dgcaip-runs/TAUSB-SDH-DGCAIP-S0-P4-SPARSE-E20-R3"
wrapper_log_root="${data_root}/tausb-dgcaip-wrapper-logs"
outer_log="${wrapper_log_root}/TAUSB-SDH-DGCAIP-S0-P4-SPARSE-E20-R3.outer.log"
launch_lock="${exp_root}-LAUNCH-LOCK"

if ! mkdir "${launch_lock}" 2>/dev/null; then
  echo "R3 launch lock already exists: ${launch_lock}" >&2
  exit 3
fi
...
mkdir -p \
  ...
  "${wrapper_log_root}"
if [[ -e "${outer_log}" ]]; then
  echo "R3 outer log already exists: ${outer_log}" >&2
  exit 3
fi
...
/usr/bin/timeout --signal=TERM --kill-after=60s 7200s \
  ...
  --device 0 \
  --mechanism-config "${repository_root}/ue_project/ue_framework/configs/tausb_sdh_dgcaip_p4_sparse_e20_r3_v1.yaml" \
