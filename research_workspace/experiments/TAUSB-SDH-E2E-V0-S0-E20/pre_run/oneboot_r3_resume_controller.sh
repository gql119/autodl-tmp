...
shutdown_once() {
  local rc=$?
  trap - EXIT INT TERM
  echo "[OneBoot-R3] controller_exit=${rc}; requesting AutoDL shutdown"
  sync || true
...
"${PYTHON_BIN}" -u -m ue_framework.tools.run_tausb_sdh_e2e_v0_oneboot \
  ...
  --device "${DEVICE}" \
  --resume-from-binding \
  --control-root /root/tausb-sdh-control/TAUSB-SDH-E2E-V0-S0-E20-ONEBOOT-R3 \
  --log-root /root/tausb-sdh-logs/TAUSB-SDH-E2E-V0-S0-E20-ONEBOOT-R3 \
  --comparison-root /root/tausb-sdh-runs/TAUSB-SDH-E2E-V0-S0-E20-COMPARISON-R3
