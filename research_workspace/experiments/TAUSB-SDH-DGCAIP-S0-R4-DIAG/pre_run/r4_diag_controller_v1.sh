...
shutdown_once() {
  ...
  if mountpoint -q "${REQUIRED_STORAGE_ROOT:-/missing}"; then
    ...
    printf '{"schema":"tausb.dgcaip-r4-wrapper-terminal.v1","exit_code":%d,"shutdown_requested":true}\n' \
      "${rc}" > "${terminal_path}"
  fi
  echo "[DGCAIP-R4] controller_exit=${rc}; requesting AutoDL shutdown"
  sync || true
...
: "${CONFIG_PATH:?reviewed R4 config is required}"
: "${ARTIFACT_ROOT:?unique R4 artifact root is required}"
...
for growing_path in \
  ...
  case "${growing_path}" in
    ...
    *) echo "[DGCAIP-R4] path outside data disk: ${growing_path}" >&2; exit 20 ;;
  esac
done
...
printf '{"schema":"tausb.dgcaip-r4-controller.v1","status":"running","execution_commit":"%s","config_sha256":"%s","hard_cap_seconds":600}\n' \
...
assert config["spec"]["spec_id"] == "TAUSB-SDH-DGCAIP-R4-D0-BINDING-FIX-v1"
assert config["dgcaip"]["run_mode"] == "r3_diag"
assert config["dgcaip"]["expected_d0_spec_id"] == "TAUSB-SDH-DGCAIP-CGR-E20-v2"
...
if [[ "${remaining}" -le 0 ]]; then
  echo "[DGCAIP-R4] hard cap exhausted during initialization" >&2
  exit 124
fi
...
printf '{"schema":"tausb.dgcaip-r4-controller.v1","status":"%s","exit_code":%d,"execution_commit":"%s","config_sha256":"%s","hard_cap_seconds":600}\n' \
