...
CHECKOUT="/root/autodl-tmp/tausb-dgcaip/checkouts/${EXECUTION_COMMIT}-r4-diag"
SESSION="tausb-dgcaip-r4-diag-s0"
PYTHON_BIN="/root/miniconda3/bin/python"
EXPECTED_CONFIG_SHA256="7143daef47321275661081a6118f22ff30b0e5d922ba968aaf061d1c0a3b2004"
ARTIFACT_ROOT="/root/autodl-tmp/tausb-dgcaip-runs/TAUSB-SDH-DGCAIP-S0-R4-DIAG"
CONTROL_ROOT="/root/autodl-tmp/tausb-dgcaip-control/TAUSB-SDH-DGCAIP-S0-R4-DIAG"
CACHE_ROOT="/root/autodl-tmp/tausb-dgcaip-cache/TAUSB-SDH-DGCAIP-S0-R4-DIAG"
TMP_ROOT="/root/autodl-tmp/tausb-dgcaip-tmp/TAUSB-SDH-DGCAIP-S0-R4-DIAG"
OUTER_LOG="/root/autodl-tmp/tausb-dgcaip-logs/TAUSB-SDH-DGCAIP-S0-R4-DIAG.log"
PRELAUNCH_FAILURE_LOG="/root/autodl-tmp/tausb-dgcaip-logs/TAUSB-SDH-DGCAIP-S0-R4-DIAG-prelaunch-failure.log"
FAILED_LINE="unknown"
...
shutdown_on_prelaunch_failure() {
  ...
  if mountpoint -q "${REQUIRED_STORAGE_ROOT}"; then
    ...
    {
      printf 'schema=tausb.dgcaip-r4-prelaunch-failure.v1\n'
      printf 'exit_code=%q\n' "${rc}"
  ...
  echo "[DGCAIP-R4-Launch] prelaunch failed with exit ${rc}; requesting shutdown" >&2
  sync || true
...
WRAPPER="${CHECKOUT}/research_workspace/experiments/TAUSB-SDH-DGCAIP-S0-R4-DIAG/pre_run/r4_diag_controller_v1.sh"
CONFIG_PATH="${CHECKOUT}/ue_project/ue_framework/configs/tausb_sdh_dgcaip_r4_d0_binding_fix_v1.yaml"
test -f "${WRAPPER}"
test -f "${CONFIG_PATH}"
...
trap - ERR EXIT INT TERM
echo "[DGCAIP-R4-Launch] handed off to tmux session ${SESSION}"
echo "[DGCAIP-R4-Launch] outer log: ${OUTER_LOG}"
echo "[DGCAIP-R4-Launch] status: ${CONTROL_ROOT}/controller_status.json"
